...
client_dir=/opt/ServerStatus/client
server_dir=/opt/ServerStatus/server
...
Environment="RUST_BACKTRACE=1"
WorkingDirectory=${working_dir}
ExecStart=$client_file -a "${PROTOCOL}://${MASTER}" -g ${USER} -p ${PASSWD}
ExecReload=/bin/kill -HUP $MAINPID
Restart=on-failure
...
function install_client() {
    rm -rf /tmp/stat_*
    rm -rf /tmp/config.toml
    echo -e "${Info} 下载 ${arch} 二进制文件 ${client_dir}"
    ...
    case ${INCMD} in
        ...
        --client|-c)
            if [ ! "$#" = 0 ]; then
                ...
                rm -rf /tmp/stat_*
                rm -rf /tmp/config.toml
                rm -rf /opt/ServerStatus/.server_status_sys_id
                echo -e "${Info} 下载 ${arch} 二进制文件"
                [ -f "/tmp/stat_client" ] || get_status '-c'
                mkdir -p ${client_dir}
                mv $tmp_client_file $client_file
